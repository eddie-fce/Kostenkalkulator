...
Ein kleines, clientseitiges Tool zur Kalkulation von 3D-Druckaufträgen mit mehreren Produkten (Positionen), Filament-Stammdaten, automatischem Mengenrabatt und CSV-Export als Preisangebot.
...
- **Stammdaten**
  ...
  - Allgemeine Kosten: Strompreis (€/kWh), Druckerleistung (W), Wartung/Verschleiß (€/Druckstunde), Arbeitskosten (€/Stunde)
  - Mengenrabatt-Stufen ("ab X Stück Y % Rabatt")
- **Kalkulation**
  - Beliebig viele Positionen (Produkte) pro Auftrag, je mit bis zu 4 Filamenten, Druckzeit, Arbeitszeit und Stückzahl
...
Alle Stammdaten (Filamente, allgemeine Kosten, Mengenrabatt-Stufen) werden automatisch im `localStorage` des Browsers gespeichert – lokal auf dem jeweiligen Gerät, ohne Server. Positionen einer Kalkulation werden bewusst **nicht** dauerhaft gespeichert, da sie sich pro Auftrag unterscheiden.
...
├── style.css    Styling
├── script.js    Anwendungslogik (Stammdaten, Kalkulation, CSV-Export, Speicher-Adapter)
├── README.md
└── LICENSE
```
